...
export const CalendarProvider = ({ children }: {children: ReactNode}) => {
  ...
  const addEvent = (date: Date, newEvent: Event) => {
    ...
    const allDayEvent = calendarDays[`${dayKey}`].events.filter((event)=>event.isAllDay)
    const nonAllDayEvent = calendarDays[`${dayKey}`].events.filter((event)=>!event.isAllDay)

    if(newEvent.isAllDay) {
        newCalendarDays[`${dayKey}`] = {date: date, events: [...allDayEvent, newEvent, ...nonAllDayEvent]}
        updateCalendarDays(newCalendarDays)
        return
    }

    const newNonAllDayEvent = [...nonAllDayEvent, newEvent].sort((a,b)=>isBefore(a.eventStartTime, b.eventStartTime)? 1: -1)
    newCalendarDays[`${dayKey}`] = {date: date, events: [...allDayEvent, ...newNonAllDayEvent]}
    updateCalendarDays(newCalendarDays)
  }
  ...
  const editEvent = (date: Date, updateEvent: Event) => {
    ...
    if(calendarDays[dayKey]) {
        ...
        const editEventIndex = newEvents.findIndex((event)=>event.id === updateEvent.id)
  ...
  const removeEvent = (date: Date, deleteEvent: Event) => {
    ...
    if(calendarDays[dayKey]) {
        newCalendarDays[`${dayKey}`].events = newCalendarDays[`${dayKey}`].events.filter((event)=>event.id !== deleteEvent.id)
        updateCalendarDays(newCalendarDays)
        return
    }
  }
  ...
  return (
    <CalendarContext.Provider value={{calendarDays, addEvent, editEvent, removeEvent}}>
      {children}
